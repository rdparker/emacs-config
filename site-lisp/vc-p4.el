...
(defun vc-p4-registered (file)
  ...
  (let ((dir (vc-p4-find-p4config (file-name-directory file))))
    (if (and vc-p4-require-p4config
	     (getenv "P4CONFIG")
	     (not dir))
	nil
      (let* ((name (file-relative-name file dir))
	     (fstat (p4-lowlevel-fstat name nil t))
...
compare non-open files to the depot version."
  (if (and (not force) (vc-file-getprop file 'vc-p4-did-fstat))
    ...
    (let* ((name (file-relative-name file))
	   (alist (or fstat-list (p4-lowlevel-fstat name nil t))))
      (if (null alist)
          'unregistered
        (let* (
               ...
               (state 
                (cond
                 ((string= action "delete")
                  'removed)
                 (action
                  (let ((opened (p4-lowlevel-opened name)))
          ...
          (vc-file-setprop file 'vc-name name)
